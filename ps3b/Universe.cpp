...
void Universe::step(double dt) {
    ...
    for (auto &entity1 : spaceEntities) {
        double netFx = 0.0, netFy = 0.0;
        for (auto &entity2 : spaceEntities) {
            if (&entity1 != &entity2) {
                double dx = entity2.getPositionX() - entity1.getPositionX();
                double dy = entity2.getPositionY() - entity1.getPositionY();
                double distance = std::sqrt(dx*dx + dy*dy);
                double force = (G * entity1.getMass() * entity2.getMass()) / (distance * distance);
                netFx += force * (dx / distance);
                netFy += force * (dy / distance);
            }
        }
        entity1.updateVelocity(netFx, netFy, dt);
    }
    for (auto &entity : spaceEntities) {
